...
public class BubbleSprite
    ...
    public void saveState(DataOutputStream data, Vector savedSprites)
        ...
        data.writeDouble(realX);
        data.writeDouble(realY);
        data.writeBoolean(fixed);
        data.writeBoolean(blink);
        data.writeBoolean(released);
        data.writeBoolean(checkJump);
        data.writeBoolean(checkFall);
        data.writeInt(fixedAnim);
        data.writeBoolean(bubbleFace == frozenFace);
    }
    ...
    public void move() {
        ...
        if (realX >= 414.) {
            moveX = -moveX;
            realX += (414. - realX);
            soundManager.playSound(SoundManager.SOUND_REBOUND);
        }
        else if (realX <= 190.) {
            moveX = -moveX;
            realX += (190. - realX);
        ...
        if (checkCollision(neighbors) || realY < 44. + frozen.getMoveDown()) {
            ...
            if (jumps.size() >= 3) {
                ...
                moveY = 0.;
                fixedAnim = 0;
                soundManager.playSound(SoundManager.SOUND_STICK);
            }
        }
    ...
    public final void paint(Graphics g, double scale, int dx, int dy) {
        ...
        if (blink && bubbleFace != frozenFace) {
            blink = false;
            drawImage(bubbleBlink, p.x, p.y, g, scale, dx, dy);
        }
        else {
            if (SettingsManager.isNormalMode() || bubbleFace == frozenFace) {
                drawImage(bubbleFace, p.x, p.y, g, scale, dx, dy);
            }
            else {
                drawImage(bubbleBlindFace, p.x, p.y, g, scale, dx, dy);
            }
        }

        if (fixedAnim != -1) {
            drawImage(bubbleFixed[fixedAnim], p.x, p.y, g, scale, dx, dy);
            fixedAnim++;
            if (fixedAnim == 6) {
                fixedAnim = -1;
            }
